...
test("le carrousel éditorial expose tradition, gastronomie et contemporain", () => {
  const pools = editorialPools(db);
  assert.equal(pools.food[0].label, "Gastronomie");
  assert.equal(pools.tradition[0].id, "c1");
  assert.equal(pools.contemporary[0].id, "c1");
  assert.equal(buildExploreEditorial({ db, date: "2026-09-06" }).tradition.id, "c1");
});
