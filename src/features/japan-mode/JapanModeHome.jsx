...
export function JapanModeHome({C,model,db,onOpenTrip,onSos,onPhrases,onToggle,onTutor}) {
  ...
  return <div style={{padding:"0 20px 110px",marginTop:-24,position:"relative",zIndex:2,display:"grid",gap:13}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><button onClick={onSos} style={{...button,background:C.red,color:"#fff",fontWeight:700}}>SOS Japon<br/><span style={{fontSize:10,fontWeight:400}}>Montrer une phrase</span></button><button onClick={onPhrases} style={button}>Phrases utiles<br/><span style={{fontSize:10,color:C.t3}}>Restaurant, train, hôtel…</span></button></div>
